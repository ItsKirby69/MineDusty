...
public class TheiaPlanetGenerator extends PlanetGenerator{
	...
	public void getColor(Vec3 position, Color out) {
		...
		// For deserts
		boolean isDesert = (desertBiome * depth )> 0.32 && height < 0.42f && height > waterLevel + 0.1f && Math.abs(position.y) < 0.37;
		if (isDesert){
			if (getSlope(position, 0.065f) > 0.074f){
				out.set(desert1).lerp(desert2, Mathf.clamp(Mathf.round(desertBiome, 0.22f))).a(0.8f);
				return;
			}
			out.set(beaches1).lerp(beaches2, height * 1.5f);
			return;
		}

		// For the Divine Faction
		if (!isDesert && divineBiome * (pole * 0.4f) * divineMask > 0.1){
		...
		if(height > waterLevel){
			...
			/// Beaches
				...
				if (basalts + (Math.abs(position.y) * 0.4f) > 0.28){
					out.set(basalts1).lerp(basalts2, Mathf.clamp(Mathf.round(depth, 0.15f))).a(1.2f);
					return;
				}
				if (pole + height > 0.3) {
